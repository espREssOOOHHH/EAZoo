...
GeneticAlgorithm::Chromosome GeneticAlgorithm::encoder(unsigned int Dimension,
	const std::vector<double>& Variables, const std::vector<struct GeneticAlgorithm::Bound>& Bounds)
{
	if (Dimension < 1)
	...
	sub_length.resize(Dimension);// length of each variable in chromosome
...
std::vector<double> GeneticAlgorithm::decoder(unsigned int Dimension,
	const std::vector<struct GeneticAlgorithm::Bound>& Bounds, const std::string& Chromosome)
{
	if (Dimension < 1)
		throw std::logic_error("Variable dimension cannot smaller than 1!");
	...
	sub_length.reserve(Dimension);// length of each variable in chromosome
	for (int i = 0; i < Dimension; i++)
	{
		auto length_interval = Bounds[i].UpperBound - Bounds[i].LowerBound;
		sub_length[i] = std::ceil(std::log(length_interval / Bounds[i].Precision) / std::log(2));
...
std::vector<GeneticAlgorithm::Chromosome> GeneticAlgorithm::roulette_selection(std::vector<GeneticAlgorithm::Chromosome>& Chromosome_Sets)
{
	...
	std::vector<GeneticAlgorithm::Chromosome> return_set(Chromosome_Sets.size());
...
int GeneticAlgorithm::crossover(std::vector<GeneticAlgorithm::Chromosome>& chromosomeSet)
{
	...
	auto try_cross = [&](auto i) ->bool{
		GeneticAlgorithm::Chromosome& chromosome_left = chromosomeSet[i * 2];
		GeneticAlgorithm::Chromosome& chromosome_right = chromosomeSet[i * 2 + 1];
		
		//select crossover point
		struct GeneticAlgorithm::Bound bound { 1, chromosome_left.str.length()-1, 1 };
...
int GeneticAlgorithm::mutation(std::vector<GeneticAlgorithm::Chromosome>& chromosomeSet)
{
...
std::vector<double> GeneticAlgorithm::get_random_numbers(unsigned int N, struct GeneticAlgorithm::Bound bound)
{
...
GeneticAlgorithm::execute(std::function<double (std::vector<double>)> EvalFuction,
	std::vector<struct GeneticAlgorithm::Bound> BoundsOfVariables,long MaxEvolutionTime,
	...
	bool DoStatistics)
{
	...
	std::vector<GeneticAlgorithm::Chromosome> chromosomeSet(population_size);
